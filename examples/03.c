...
static const char *SOURCE_BYTECODE =
    "i = 0\n"
    "WHILE i < n\n"
    "  temp = a + b\n"
    "  i = i + 1\n"
...
static const char *SOURCE_NATIVE =
    "i = 0\n"
    "WHILE i < n\n"
    "  NADD a, b, $temp\n"
    "  i = i + 1\n"
...
static basic26_FunctionResult nadd_function(const basic26_CallInfo *info, size_t argc, const basic26_Value *argv)
{
    g_native_call_count++;

    if (argc != 3 || argv[0].type != BASIC26_VALUE_TYPE_INT || argv[1].type != BASIC26_VALUE_TYPE_INT || argv[2].type != BASIC26_VALUE_TYPE_SYMBOL)
    {
    ...
    CHECK(basic26_State_set_var(info->state, argv[2].as.symbol_id, &(basic26_Value){
...
static basic26_FunctionResult noop_function(const basic26_CallInfo *info, size_t argc, const basic26_Value *argv)
{
    UNUSED(info);
    UNUSED(argc);
    UNUSED(argv);
...
// `prepare` is called before each run to reset state.
...
// Returns per-iteration timings (ns) for `iterations` runs of the given script.
// Caller owns the returned array.
static double *bench_script_per_iter(basic26_Script *script, int iterations)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        const long long t0 = basic26_now_ns();
        basic26_run_sync(vm, state, script);
        const long long t1 = basic26_now_ns();
...
static void report_variant(const char *label,
                           basic26_Script *script)
{
    ...
    basic26_Stats s = basic26_compute_stats(runs, RUNS);
...
int main(void)
{
    ...
    printf("  native  : temp = %lld  (calls this run: %lld)\n",
           (long long)r_nat, (long long)nat_calls);
    ...
    report_variant("bytecode", script_bytecode);
    printf("\n");
    report_variant("native", script_native);
    printf("\n");
    report_variant("noop", script_noop);
    printf("\n");

    // ---- Per-iteration detail for the last variant ---------------------
    double *per_iter = bench_script_per_iter(script_native, BENCH_ITERATIONS);
    basic26_Stats s = basic26_compute_stats(per_iter, BENCH_ITERATIONS);
    basic26_print_stats("native (per-iter, last run)", &s);
    free(per_iter);
    printf("\n");
    ...
    double bc_mean = bench_script(script_bytecode, BENCH_ITERATIONS);
    double nat_mean = bench_script(script_native, BENCH_ITERATIONS);
    double nop_mean = bench_script(script_noop, BENCH_ITERATIONS);
    ...
    printf("Empty native call throughput                          : %.0f calls/sec\n", noop_throughput);
    printf("NADD native call throughput (incl. callback body)     : %.0f calls/sec\n", nadd_throughput);
