...
//
// OVERVIEW OF THE BASIC26 INTERPRETER AND LANGUAGE
// -------------------------------------------------
// BASIC26 is a lightweight, embeddable scripting language with a C API. The
// host application compiles source text into bytecode and then executes it on
// a stack-based virtual machine. There is no standalone interpreter binary;
// the VM is always embedded inside a host program.
//
// Core concepts:
//
//   Vm      - The top-level object. Owns the string table and the registry of
//             native function callbacks. Created once and shared across scripts.
//
//   Script  - A compiled bytecode blob. Source text is parsed line-by-line into
//             opcodes. A script can be compiled once and executed many times.
//
//   State   - The execution context for a single run. Holds the value stack,
//             variable store, and instruction pointer. Can be reset and reused.
//
// Language features:
//
//   Data types:
//     INT     - 64-bit signed integer (e.g. 42, -7, 0)
//     FLOAT   - 64-bit double (e.g. 3.14, NAN, INF)
//     STRING  - interned string literal (e.g. "hello")
//     SYMBOL  - identifier literal prefixed with $ (e.g. $foo)
//     NULL    - the null value
//     OBJECT  - opaque host pointer, managed entirely by native callbacks
//
//   Variables:
//     Variables are NOT created implicitly by the interpreter. All variables
//     must be explicitly created by the host application via
//     basic26_State_set_var() before the script references them. The
//     interpreter does not allocate memory for variables on its own  - variable
//     creation is entirely the host's responsibility. Once created, a script
//     can assign any type to the variable:
//       x = 10
//       name = "Alice"
//
//   Expressions and operators (lowest to highest precedence):
//     OR               - boolean OR              (INT, INT) -> INT
//     AND              - boolean AND             (INT, INT) -> INT
//     == !=            - equality / inequality   (same type) -> INT
//     < > <= >=        - comparison              (INT/FLOAT) -> INT
//     | ^ &            - bitwise OR/XOR/AND      (INT, INT) -> INT
//     << >>            - bitwise shift            (INT, INT) -> INT
//     + -              - add / subtract           (INT or FLOAT) -> same
//     * / %            - multiply / divide / rem  (INT or FLOAT) -> same
//     NOT              - boolean NOT (unary)      INT -> INT
//     ~                - bitwise NOT (unary)      INT -> INT
//
//   Control flow:
//     IF expr ... ELSE ... ENDIF    - conditional
//     WHILE expr ... ENDWHILE      - pre-condition loop
//     GOTO label                   - unconditional jump to a label
//     label_name:                  - label definition
//
//   Function calls:
//     All functions are native callbacks registered by the host. A call looks
//     like:  FUNC arg1, arg2, arg3
//     Arguments are comma-separated and evaluated left-to-right.
//     There is no return-value mechanism; callbacks communicate results back
//     to the script by setting variables.
//
//   Comments:
//     Lines beginning with // are ignored. Inline trailing comments are also
//     supported.
//
//   Sandbox / safety:
//     The host can set execution limits (max opcodes, max wall-clock time)
//     to prevent runaway scripts. A native callback can also YIELD to suspend
//     execution, allowing the host to resume it later.
//
//   StringId / SymbolId interchangeability:
//     Both types share the same interning table. A given character sequence
//     always produces the same numeric ID regardless of which type alias is
//     used. The distinction exists solely for semantic clarity. You can safely
//     pass a StringId where a SymbolId is expected and vice versa.
//
//   _clear functions:
//     The _clear functions (basic26_Vm_clear, basic26_State_clear,
//     basic26_Script_clear) do NOT free memory. They only reset internal data
//     structures while retaining their allocated capacity. This enables maximum
//     memory reuse and avoids unnecessary allocation/deallocation cycles and
//     fragmentation. Any cleared object can be reused without recreating it.
//     WARNING: clearing interned strings or labels can break the interpreter
//     (runtime errors), though it will not cause a crash.
//
