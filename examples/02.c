...
int main(void) {
  ...
  printf("fib(%d) = %lld (expected %lld) %s\n", FIB_N, (long long)result,
         (long long)expected, result == expected ? "OK" : "MISMATCH");
  ...
  basic26_Stats overall = basic26_compute_stats(run_results, RUNS, T_CRITICAL);
