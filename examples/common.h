...
basic26_runtime_error_to_string(basic26_RuntimeError code) {
  switch (code) {
...
static inline void basic26_set_int_var(basic26_State *st, basic26_SymbolId sym,
                                       int64_t val) {
  CHECK(basic26_State_set_var(st, sym,
                              &(basic26_Value){
...
static inline int64_t basic26_get_int_var(basic26_State *st,
                                          basic26_SymbolId sym) {
  basic26_Value out;
...
static inline basic26_SymbolId basic26_intern(basic26_Vm *vm,
                                              const char *name) {
  basic26_SymbolId id;
  CHECK(basic26_Vm_get_string_id(vm, (const uint8_t *)name, strlen(name), true,
                                 &id) == BASIC26_RESULT_OK);

  return id;
}

static inline void basic26_register(basic26_Vm *vm, const char *name,
                                    basic26_FunctionCallback cb) {
...
static inline void basic26_compile(basic26_Vm *vm, basic26_Script *script,
                                   const char *source) {
  UNUSED(vm);

  basic26_CompileErrorInfo err;
  CHECK(basic26_Script_compile(script,
...
static inline void basic26_run_sync(basic26_Vm *vm, basic26_State *state,
                                    basic26_Script *script) {
  basic26_RuntimeErrorInfo err;
  CHECK(basic26_Vm_run(vm,
                       &(basic26_RunOptions){
                           .state = state,
                           .script = script,
...
// Compute mean / median / min / max / std_dev / sem / ci_err_margin over a
// sample. Sorts `values` in place.
static inline basic26_Stats basic26_compute_stats(double *values, size_t n,
  ...
  s.std_dev = sqrt(variance / (double)(n - 1));
  s.sem = s.std_dev / sqrt(n);
  s.ci_err_margin = t_critical * s.sem;

  return s;
}

static inline void basic26_print_stats(const char *label,
                                       const basic26_Stats *s) {
  printf("--- %s (ns/iter) ---\n", label);
  printf("Mean:     %.3f ns\n", s->mean);
  printf("Median:   %.3f ns\n", s->median);
  printf("Min:      %.3f ns\n", s->min);
  printf("Max:      %.3f ns\n", s->max);
  printf("Std Dev:  %.3f ns (%.3f%%)\n", s->std_dev,
         (s->std_dev / s->mean) * 100.0);
  printf("SEM:      %.3f ns\n", s->sem);
  printf("95%% CI:   %.3f ns ± %.3f\n", s->mean, s->ci_err_margin);
}
